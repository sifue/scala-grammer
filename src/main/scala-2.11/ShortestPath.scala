/**
  * Created by soichiro_yoshimura on 2016/06/22.
  */
case class Edge(from: Char, to: Char, cost: Int)
...
object ShortestPath {
    ...
    while (isUpdated) {
      ...
      edges.foreach { e =>
        if(distances(e.from) != Int.MaxValue
          ...
          distances = distances + (e.to -> (distances(e.from) + e.cost))
          isUpdated = true
        }
      }
    }
  ...
  def solveByWarshallFloyd(start: Char, goal: Char): Unit = {
    ...
    distanceMap = distanceMap ++ edges.map(e => (e.from, e.to) -> e.cost)
